...
· Keyboard Shortcuts — Space to play/pause, Ctrl+Z to undo, Ctrl+Shift+Z to redo`,
    ...
    {
      id: 'audio-converter',
      title: 'Конвертер аудиоформатов',
      overview: `Конвертер аудиоформатов — это специализированный инструмент для преобразования аудиоформатов. Импортируйте аудиофайлы (MP3, WAV, OGG, FLAC, M4A, AAC, WEBM) и конвертируйте их в широкий спектр выходных форматов с полным контролем над параметрами кодирования. Инструмент обрабатывает аудио полностью в браузере с помощью Web Audio API — загрузка на сервер не требуется.
...
· Error Panel — Detailed error messages with codes, causes, and solutions`,
    ...
    },
    {
      ...
      overview: `Сеть отношений — это интерактивный визуальный редактор графов для отображения связей между оригинальными персонажами (ОС). Создавайте узлы персонажей с именами, цветами, заметками и аватарами из галереи активов, а затем связывайте их цветными линиями отношений.

Основные возможности:
· Визуальный холст — свободно панорамируйте, масштабируйте и перетаскивайте узлы на бесконечном холсте
· 8 типов отношений — Друг, Враг, Семья, Возлюбленный, Наставник, Соперник, Союзник и Другое, каждый со своим цветом
· Редактор узлов — задайте имя, выберите цвет из палитры, добавьте заметки и назначьте аватар из галереи активов
· Редактор связей — выберите тип отношения, добавьте опциональную метку и заметки
· Интеграция с галереей активов — импортируйте иллюстрации персонажей напрямую как аватары узлов
· Управление холстом — приближение/отдаление, сброс вида, горячие клавиши (Delete — удалить, Escape — снять выделение)
· Персистентность — все узлы и связи автоматически сохраняются в localStorage браузера
· Доступность с клавиатуры — узлы доступны через Tab, Enter/Space — выбрать, Shift+Enter — быстро добавить связь`,
      buttons: [
        { name: 'Добавить персонажа', description: 'Создать новый узел персонажа в центре текущего видового экрана.' },
        { name: 'Добавить связь', description: 'Активировать режим создания связи. Сначала кликните исходный узел, затем целевой — связь будет создана.' },
        { name: 'Редактировать', description: 'Открыть модальное окно редактора узла или связи, чтобы изменить имя, цвет, аватар, тип, метку и заметки.' },
        { name: 'Удалить', description: 'Удалить выбранный узел или связь. При удалении узла удаляются и все присоединённые к нему связи.' },
        { name: 'Приблизить / Отдалить', description: 'Изменить масштаб холста. Также поддерживается колёсико мыши.' },
        { name: 'Сбросить вид', description: 'Сбросить панорамирование и масштаб к стандартному центрированному виду.' },
        { name: 'Выбрать аватар', description: 'Открыть выборщик галереи активов, чтобы назначить изображение аватаром узла.' },
        { name: 'Убрать аватар', description: 'Удалить изображение аватара с узла, вернув заглушку с первой буквой имени.' },
      ],
      parameters: [
        { name: 'Имя персонажа', description: 'Отображаемое имя узла персонажа.', tips: 'Держите коротким, чтобы метка помещалась под кругом узла.' },
        { name: 'Цвет узла', description: 'Акцентный цвет узла, отображаемый как фон аватара и кольцо выделения.', tips: 'Выбирайте цвета с хорошим контрастом относительно фона холста.' },
        { name: 'Заметки узла', description: 'Опциональные текстовые заметки, прикреплённые к узлу.', tips: 'Используйте для предыстории персонажа, черт или напоминаний.' },
        { name: 'Тип связи', description: 'Категория отношений между двумя персонажами.', tips: 'Выберите «Другое», если ни один из предустановленных типов не подходит.' },
        { name: 'Метка связи', description: 'Опциональная короткая метка, отображаемая вдоль линии связи.', tips: 'Примеры: «Друг детства», «Заклятый враг с 2020 г.».' },
        { name: 'Заметки связи', description: 'Опциональные текстовые заметки, прикреплённые к отношению.', tips: 'Используйте для истории отношений, событий или сюжетного контекста.' },
      ],
      errors: [
        {
          code: 'NO_NODES',
          message: 'В графе пока нет персонажей',
          severity: 'info',
          category: 'B. Настройки и данные',
          location: 'Страница: Сеть отношений → Область: Холст',
          cause: 'Граф пуст, потому что ещё не создано ни одного узла.',
          solution: 'Нажмите «Добавить персонажа», чтобы создать первый узел.',
          steps: ['Нажмите кнопку Добавить персонажа', 'Введите имя', 'Нажмите Сохранить'],
          relatedCodes: [],
          prevention: 'N/A',
        },
        {
          code: 'STORAGE_FULL',
          message: 'Хранилище браузера заполнено',
          severity: 'error',
          category: 'F. Браузер и производительность',
          location: 'Страница: Сеть отношений → Область: Сохранение',
          cause: 'Превышена квота localStorage браузера, узлы и связи не могут быть сохранены.',
          solution: 'Освободите место в хранилище браузера или экспортируйте данные для резервного копирования.',
          steps: ['Очистите неиспользуемые данные localStorage', 'Экспортируйте данные сети', 'Перезагрузите страницу'],
          relatedCodes: ['DATA_CORRUPTED'],
          prevention: 'Регулярно создавайте резервные копии данных сети отношений.',
        },
      ],
    },
    {
      ...
      overview: `Экспортёр карточек персонажей позволяет объединить информацию о персонаже, иллюстрации, поля профиля, теги и данные об отношениях в красивую, доступную для публикации PNG-карточку. Выбирайте из трёх шаблонов (Минимальный, Подробный, Галерея), настраивайте цвета темы и фон, а затем экспортируйте одним щелчком.

Основные возможности:
· Три шаблона карточек — Минимальный (компактная портретная карточка), Подробный (полный профиль с главным изображением), Галерея (центрированная витрина с сеткой отношений)
· Интеграция с галереей активов — Выбирайте аватар и главное изображение прямо из галереи активов
· Интеграция с сетью отношений — По желанию автоматически импортируйте связанных персонажей и типы их отношений
· Пользовательские поля профиля — Добавляйте любое количество помеченных полей (возраст, рост, день рождения, личность и т.д.)
· Теги и цвета — Назначайте цветные теги для категоризации вашего персонажа
· Настройка темы — Выбирайте акцентный цвет из 19 пресетов; выбирайте стиль фона (сплошной, градиент или точечный узор)
· Экспорт в один клик — Рендерит карточку в 2× разрешении через html-to-image и скачивает как PNG`,
      buttons: [
        { name: 'Экспорт карточки', description: 'Отрендерить текущий предпросмотр карточки как PNG-изображение высокого разрешения и скачать на устройство.' },
        { name: 'Выбрать аватар', description: 'Открыть выборщик галереи активов, чтобы выбрать квадратное изображение аватара для карточки.' },
        { name: 'Выбрать главное изображение', description: 'Открыть выборщик галереи активов, чтобы выбрать главное/баннерное изображение для шаблонов Подробный и Галерея.' },
        { name: 'Добавить тег', description: 'Создать новый цветной тег. Цвет назначается автоматически случайным образом.' },
        { name: 'Добавить поле', description: 'Добавить новое пользовательское поле профиля с меткой и значением.' },
        { name: 'Удалить поле / тег', description: 'Удалить определённое поле профиля или тег с карточки.' },
        { name: 'Переключить шаблон', description: 'Переключаться между макетами карточек Минимальный, Подробный и Галерея.' },
        { name: 'Стиль фона', description: 'Переключаться между сплошным фоном, наложением градиента акцентного цвета и наложением точечного узора.' },
      ],
      parameters: [
        { name: 'Имя персонажа', description: 'Основное отображаемое имя персонажа на карточке.', tips: 'Обязательное поле. Кнопка экспорта отключена, пока не введено имя.' },
        { name: 'Псевдоним', description: 'Опциональное вторичное имя или прозвище.', tips: 'Оставьте пустым, если у персонажа нет псевдонима.' },
        { name: 'Цвет темы', description: 'Акцентный цвет, используемый для рамок, выделений и градиентных фонов.', tips: 'Выберите цвет, который дополняет иллюстрацию персонажа.' },
        { name: 'Биография персонажа', description: 'Свободное текстовое описание или предыстория, отображаемая на карточке.', tips: 'Держите кратко для шаблона Минимальный; шаблоны Подробный и Галерея имеют больше места.' },
        { name: 'Импорт отношений', description: 'При включении инструмент считывает данные сети отношений и автоматически заполняет связанных персонажей.', tips: 'Инструмент ищет узел, имя которого точно совпадает с полем «Имя персонажа».' },
        { name: 'Шаблон', description: 'Визуальный макет экспортируемой карточки.', tips: 'Подробный лучше всего подходит для полных профилей; Галерея лучше всего подходит для визуально ориентированных витрин.' },
      ],
      errors: [
        {
          code: 'NO_NAME',
          message: 'Имя персонажа пусто',
          severity: 'warning',
          category: 'B. Настройки и данные',
          location: 'Страница: Карточка персонажа → Область: Кнопка экспорта',
          cause: 'Кнопка экспорта была нажата до ввода имени персонажа.',
          solution: 'Введите хотя бы один символ в поле «Имя персонажа».',
          steps: ['Нажмите на поле ввода имени персонажа', 'Введите имя', 'Снова нажмите кнопку Экспорт карточки'],
          relatedCodes: [],
          prevention: 'N/A',
        },
        {
          code: 'EXPORT_FAILED',
          message: 'Ошибка экспорта карточки',
          severity: 'error',
          category: 'F. Браузер и производительность',
          location: 'Страница: Карточка персонажа → Область: Экспорт',
          cause: 'Браузер заблокировал рендеринг изображения, DOM предпросмотра был отсоединён, или html-to-image столкнулся с изображением из другого источника.',
          solution: 'Убедитесь, что все выбранные изображения из галереи активов (Data URL). Попробуйте повторно выбрать изображения или перезагрузите страницу.',
          steps: ['Проверьте, что изображения всё ещё есть в галерее активов', 'Повторно выберите аватар/главное изображение', 'Снова нажмите кнопку Экспорт'],
          relatedCodes: [],
          prevention: 'Используйте только изображения, импортированные в галерею активов, чтобы избежать проблем с CORS.',
        },
      ],
    },
    {
      ...
      overview: `Character Chronicle позволяет создать визуальную шкалу ключевых событий из жизни персонажа. Записывайте рождения, встречи, расставания, моменты взросления, битвы и поворотные точки — каждое с датой, описанием, привязанными персонажами из Сети отношений и опциональными изображениями из Галереи ресурсов. Шкала времени отображается в виде красивой вертикальной оси с цветными карточками событий, которую можно экспортировать как длинное PNG-изображение.

Основные возможности:
· Создание событий — добавление событий с датой, названием, описанием, типом и опциональным изображением
· Связь с Сетью отношений — привязка связанных персонажей к любому событию; их имена и цвета отображаются на карточке шкалы времени
· Цветовая кодировка типов событий — 7 встроенных типов (Рождение, Встреча, Разлука, Рост, Битва, Поворотный момент, Пользовательский), каждый со своим уникальным цветом
· Визуальная шкала времени — вертикальная ось с чередующимися карточками событий слева/справа, автоматически упорядоченными по дате
· Экспорт в один клик — рендеринг всей шкалы времени в 2× разрешении через html-to-image как прозрачное PNG
· Постоянное хранение — все события автоматически сохраняются в localStorage браузера`,
      buttons: [
        { name: 'Add Event', description: 'Открыть модальное окно редактора событий для создания новой записи на шкале времени.' },
        { name: 'Edit Event', description: 'Открыть существующее событие в модальном окне редактора для изменения его полей.' },
        { name: 'Delete Event', description: 'Удалить событие со шкалы времени после подтверждения.' },
        { name: 'Export Timeline', description: 'Отрендерить всю шкалу времени как PNG высокого разрешения и скачать её.' },
        { name: 'Select Image', description: 'Выбрать изображение из Галереи ресурсов для прикрепления к событию.' },
        { name: 'Link Character', description: 'Переключить персонажа из Сети отношений как связанного с этим событием.' },
        { name: 'Event Type Chip', description: 'Нажмите на чип типа, чтобы установить категорию события и его цвет.' },
      ],
      parameters: [
        { name: 'Date', description: 'Дата события, используемая для хронологической сортировки.', tips: 'Для лучших результатов используйте формат ISO (YYYY-MM-DD). Относительные даты, такие как «15 лет», работают, но могут сортироваться неожиданно.' },
        { name: 'Title', description: 'Отображаемое название события на карточке шкалы времени.', tips: 'Держите его коротким (менее 30 символов), чтобы оно хорошо помещалось на карточке.' },
        { name: 'Description', description: 'Более развёрнутое текстовое описание того, что произошло.', tips: 'Два-три предложения — идеально для макета карточки.' },
        { name: 'Event Type', description: 'Категория, определяющая цвет события.', tips: 'Используйте Пользовательский, если ни один из предустановленных вариантов не подходит для вашего сюжетного поворота.' },
        { name: 'Related Characters', description: 'Персонажи из Сети отношений, связанные с этим событием.', tips: 'Инструмент автоматически читает узлы Сети отношений. Убедитесь, что имена персонажей совпадают.' },
      ],
      errors: [
        {
          code: 'NO_EVENTS',
          message: 'Шкала времени пуста',
          severity: 'info',
          category: 'B. Settings & Data',
          location: 'Page: Character Chronicle → Area: Timeline',
          cause: 'Ещё не создано ни одного события.',
          solution: 'Нажмите «Add Event», чтобы создать первую запись на шкале времени.',
          steps: ['Нажмите Add Event', 'Заполните дату и название', 'Нажмите Save'],
          relatedCodes: [],
          prevention: 'N/A',
        },
        {
          code: 'EXPORT_FAILED',
          message: 'Не удалось экспортировать шкалу времени',
          severity: 'error',
          category: 'F. Browser & Performance',
          location: 'Page: Character Chronicle → Area: Export',
          cause: 'Браузер заблокировал рендеринг, или была обнаружена кросс-доменная картинка.',
          solution: 'Убедитесь, что все прикреплённые изображения из Галереи ресурсов (Data URL). Попробуйте удалить изображения и экспортировать снова.',
          steps: ['Проверьте источники изображений в Галерее ресурсов', 'Удалите проблемные изображения из событий', 'Снова нажмите Export Timeline'],
          relatedCodes: [],
          prevention: 'Используйте только изображения, импортированные в Галерею ресурсов, чтобы избежать проблем с CORS.',
        },
      ],
    },
    {
      ...
      overview: `Энциклопедия мира — это структурированный инструмент управления знаниями для создания оригинального мира. Создавайте записи о местах, организациях, расах, событиях, предметах, концепциях и пользовательских категориях. Каждая запись поддерживает теги, связанных персонажей из Сети отношений и произвольный текст. Просматривайте в виде списка или сетки карточек с возможностью поиска и фильтрации по категориям. Экспортируйте всю энциклопедию в формате JSON для резервного копирования или обмена.

Ключевые возможности:
· Семь категорий записей — Место (зелёный), Организация (синий), Раса (янтарный), Событие (красный), Предмет (фиолетовый), Концепция (циан), Пользовательский (серый)
· Система тегов — Назначайте любое количество текстовых тегов для категоризации и фильтрации записей
· Связь с Сетью отношений — Прикрепляйте связанных персонажей к любой записи; их цвета отображаются точками на карточке
· Два режима просмотра — Сетка карточек для визуального просмотра, компактный список для быстрого сканирования
· Поиск и фильтрация — Полнотекстовый поиск по заголовкам, содержимому и тегам; категории с живыми счётчиками
· Экспорт JSON — Копируйте всю энциклопедию в буфер обмена как отформатированный JSON
· Постоянное хранение — Все записи автоматически сохраняются в локальном хранилище браузера`,
      buttons: [
        { name: 'Добавить запись', description: 'Открыть модальное окно редактора для создания новой записи энциклопедии.' },
        { name: 'Редактировать запись', description: 'Открыть существующую запись в редакторе для изменения полей.' },
        { name: 'Удалить запись', description: 'Удалить запись из энциклопедии после подтверждения.' },
        { name: 'Экспорт JSON', description: 'Скопировать весь набор данных энциклопедии в буфер обмена как отформатированный JSON.' },
        { name: 'Поиск', description: 'Отфильтровать записи, вводя ключевые слова, соответствующие заголовкам, содержимому или тегам.' },
        { name: 'Фильтр категорий', description: 'Нажмите на категорию, чтобы показать только записи этого типа. Нажмите снова, чтобы сбросить фильтр.' },
        { name: 'Переключение вида', description: 'Переключайтесь между сеткой карточек и компактным списком.' },
        { name: 'Привязать персонажа', description: 'Переключить персонажа из Сети отношений как связанного с этой записью.' },
      ],
      parameters: [
        { name: 'Название', description: 'Отображаемое имя записи энциклопедии.', tips: 'Держите его кратким и уникальным для удобного поиска.' },
        { name: 'Категория', description: 'Классификация, определяющая цвет и значок записи.', tips: 'Используйте «Пользовательский», если ни один из шести предустановленных типов не подходит.' },
        { name: 'Содержимое', description: 'Основной текст, подробно описывающий запись.', tips: 'Поддерживает произвольный текст. Опишите лор, историю, внешность или правила.' },
        { name: 'Теги', description: 'Текстовые метки для дополнительной категоризации помимо предустановленных категорий.', tips: 'Примеры: «magic», «faction-A», «main-city», «legendary».' },
        { name: 'Связанные персонажи', description: 'Персонажи из Сети отношений, связанные с этой записью.', tips: 'Удобно для обозначения персонажей, принадлежащих организации, живущих в локации или участвовавших в событии.' },
      ],
      errors: [
        {
          code: 'NO_ENTRIES',
          message: 'Энциклопедия пуста',
          severity: 'info',
          category: 'B. Настройки и данные',
          location: 'Страница: Энциклопедия мира → Область: Область содержимого',
          cause: 'Записи ещё не созданы.',
          solution: 'Нажмите «Добавить запись», чтобы создать первую запись энциклопедии.',
          steps: ['Нажмите Добавить запись', 'Заполните название и категорию', 'Нажмите Сохранить'],
          relatedCodes: [],
          prevention: 'N/A',
        },
        {
          code: 'EXPORT_FAILED',
          message: 'Ошибка экспорта JSON',
          severity: 'error',
          category: 'F. Браузер и производительность',
          location: 'Страница: Энциклопедия мира → Область: Кнопка экспорта',
          cause: 'Браузер заблокировал доступ к буферу обмена или данные слишком велики.',
          solution: 'Убедитесь, что у страницы есть разрешение на доступ к буферу обмена. Если данные очень большие, попробуйте удалить некоторые записи перед экспортом.',
          steps: ['Проверьте разрешения браузера', 'При необходимости уменьшите количество записей', 'Снова нажмите Экспорт JSON'],
          relatedCodes: [],
          prevention: 'Регулярно экспортируйте небольшие партии, чтобы избежать слишком большого JSON.',
        },
      ],
    },
    {
