...
const translations: Record<BaseLanguage, Messages> = {
  ...
  ja: {
    ...
    featureInspirationGenerator: 'キャラインスピレーション生成',
    ...
    actionInspirationGenerator: 'インスピレーション生成',
    ...
    pageInspirationGeneratorTitle: 'キャラインスピレーション生成',
    pageInspirationGeneratorDescription: 'ランダムなキャラクター案を一括生成：性格、外見、服装、背景、口癖、欠点、秘密、趣味。ロック、お気に入り、履歴、JSON 出力に対応。',
...
const localizedMessages: Record<AppLanguage, Messages> = {
  ...
  ko: {
    ...
    featureInspirationGenerator: '캐릭터 영감 생성기',
    ...
    actionInspirationGenerator: '캐릭터 영감 생성기',
    ...
    pageInspirationGeneratorTitle: '캐릭터 영감 생성기',
    pageInspirationGeneratorDescription: '랜덤한 캐릭터 영감 콤보를 한 번에 생성: 성격, 외모, 의상, 배경, 입버릇, 결점, 비밀, 취미. 잠금, 즐겨찾기, 기록, JSON 납품 지원.',
...
const announcementHistory = [
  ...
  },
  {
    version: '1.20.2',
    date: '2026-05-18',
    title: '1.20.2 第十六轮全面深度审计：AU/宿敌工坊 UI/UX/SFX/BGM/可访问性/本地化/数据完整性',
    summary:
      '对 v1.20.0 和 v1.19.0 新增模块进行全面深度审计，修复嵌套按钮、SFX 双重播放、可访问性缺口、韩文 CJK 混用、缺失翻译键、localStorage 失败静默、定时器泄漏等关键问题。',
    details: [
      'UI/UX 修复：重构 AU 工坊与宿敌工坊的标签页结构，消除嵌套交互元素（span role=button 嵌套在 button 内）；修复模态框关闭按钮与遮罩层的 SFX 双重播放。',
      '可访问性增强：补全缺失的 ARIA 标签页语义（tablist/tab/tabpanel/aria-selected）；为滑块输入添加 aria-label；为导出模态框添加 aria-labelledby。',
      '本地化修复：彻底清理韩文翻译中的中日文汉字混用（宿敵→숙적、灵감→영감、偏移→변경、导師→교수）；修复宿敌工坊导出功能缺失的 statUp/statDown/statSame 翻译键。',
      '稳定性增强：新增 localStorage 写入失败 toast 提示、页面刷新保护（useBeforeUnloadGuard）、定时器泄漏清理与状态验证加固。',
    ],
  },
  {
    version: '1.20.1',
    date: '2026-05-18',
    title: '1.20.1 审计热修复：键名不匹配、本地化混用、UX 微调',
    summary:
      '修复角色宿敌工坊 dark-mirror 键名不匹配、韩文/俄文/日文本地化混用与翻译不当、AU 工坊模板芯片新增 title 描述提示、标签页关闭按钮触摸区域扩大。',
    details: [
      '修复 dark-mirror 键名不匹配：UI_COPY 驼峰 darkMirror → 连字符 dark-mirror，消除宿敌类型选择 undefined 显示。',
      '本地化修复：韩文混用日文汉字（宿敵→숙적）、非韩文字符（namePrefixes 混入中文）；俄文混入中文/翻译不当。',
      'UX 优化：AU 工坊模板芯片新增 title 属性显示描述提示；标签页关闭按钮触摸区域 14px→20px。',
    ],
  },
  {
    version: '1.20.0',
    date: '2026-05-18',
    title: '1.20.0 角色宿敌工坊：为原创角色自动生成 5 种宿敌类型',
    summary:
      '新增 Character Nemesis Generator 页面，支持从 5 种宿敌类型生成对立角色，原版 vs 宿敌属性对比，Markdown 导出，5 语言完整本地化。',
    details: [
      '新增角色宿敌工坊：5 种宿敌类型（性格反转/实力对手/黑暗镜像/衬托对比/命运宿敌），自动生成名字/称号/外貌/动机/弱点/背景/冲突场景/关系动态。',
      '属性对比视图：原版 vs 宿敌并排对比，红色主题，带 ↑/↓/→ 变化指示器。',
      '系统集成：types.ts 新增共享类型 CoreStat/BaseCharacter；localStorage 持久化；5 语言完整本地化；可访问性与 SFX 全覆盖。',
    ],
  },
  {
    version: '1.19.0',
    date: '2026-05-18',
    title: '1.19.0 平行宇宙工坊：为原创角色生成 8 种 AU 版本',
    summary:
      '新增 AU Generator 页面，支持从 8 种模板生成平行宇宙版本角色，原版 vs AU 属性对比，Markdown 导出，5 语言完整本地化。',
    details: [
      '新增平行宇宙工坊：8 种 AU 模板（现代校园/赛博朋克/中世纪/末日废土/魔法学院/太空殖民/蒸汽朋克/恐怖洋馆），自动生成服装/背景/风味描述。',
      '属性对比视图：原版 vs AU 并排对比，带 ↑/↓/→ 变化指示器。',
      '系统集成：懒加载路由、5 语言完整本地化、localStorage 持久化、模板芯片 title 提示。',
    ],
  },
  {
    version: '1.18.0',
