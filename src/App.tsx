...
const translations: Record<BaseLanguage, Messages> = {
  ...
  workflowFormats: '支持 PNG / JPG / WEBP，推荐上传单人立绘或清晰半身图。',
    ...
    announcementDescription: '0.3.3.1 继续做了一轮稳定性修补，把 paper2gal 的下载错误链路也接入可读诊断，并顺手完成整站回归检查。',
    announcementList1: 'paper2gal 下载单个结果文件或下载整包失败时，现在也会显示可读错误原因、后端返回内容和当前请求地址。',
    announcementList2: '模型接口地址误填检查继续覆盖启动、轮询和下载三条链路，不会再只在部分操作里提示。',
    announcementList3: '重新构建并复查了当前 UI、版本号、公告面板和本地保存逻辑，保持现有样式与多语言结构同步。',
    aboutTitle: '关于',
  ...
  ja: {
    ...
    announcementDescription: '0.3.3.1 では安定性をさらに補強し、paper2gal のダウンロード失敗も可読な診断付きで確認できるようにしました。',
    announcementList1: 'paper2gal で個別ファイルや zip 一括ダウンロードに失敗した場合も、backend 応答・原因・リクエスト URL を分かりやすく表示します。',
    announcementList2: 'モデル API の誤入力チェックは開始・ポーリング・ダウンロードの全経路に適用され、途中の操作だけ無警告になる状態を防ぎます。',
    announcementList3: '現行 UI、バージョン表示、お知らせ面板、ローカル保存まわりも再確認し、既存の見た目と多言語構成を維持しました。',
    aboutTitle: '情報',
  ...
  en: {
    ...
    announcementDescription: 'Version 0.3.3.1 adds another stability pass: paper2gal download failures now use the same readable diagnostics as workflow start and polling errors.',
    announcementList1: 'When downloading a single output file or the full archive fails, the UI now shows the readable reason, backend response, and the actual request URL.',
    announcementList2: 'Wrong model-endpoint detection now covers the start, polling, and download paths instead of warning only in part of the workflow.',
    announcementList3: 'The current UI, version labels, announcement panels, and local persistence flow were rechecked so the existing style and language structure stay in sync.',
    aboutTitle: 'About',
  ...
  ru: {
    ...
    announcementDescription: 'Версия 0.3.3.1 добавляет ещё один проход по стабильности: ошибки загрузки в paper2gal теперь тоже показываются в читаемом и понятном виде.',
    announcementList1: 'При сбое скачивания отдельного файла или полного архива paper2gal теперь показывает понятную причину, ответ backend и фактический URL запроса.',
    announcementList2: 'Проверка ошибочно введённого model endpoint теперь охватывает запуск, опрос статуса и скачивание, а не только часть цепочки.',
    announcementList3: 'Текущий UI, номер версии, панели объявлений и локальное сохранение были ещё раз проверены, чтобы сохранить синхронность стиля и языков.',
...
const localizedMessages: Record<AppLanguage, Messages> = {
  ...
  ko: {
    ...
    announcementDescription: '0.3.3.1에서는 안정성 보강을 한 번 더 진행해 paper2gal 다운로드 실패도 읽기 쉬운 진단으로 표시되게 했습니다.',
    announcementList1: '개별 결과 파일이나 전체 zip 다운로드가 실패해도 이제는 원인 요약, 백엔드 응답, 실제 요청 URL을 함께 볼 수 있습니다.',
    announcementList2: '모델 엔드포인트 오입력 감지는 시작, 상태 폴링, 다운로드 경로까지 모두 커버합니다.',
    announcementList3: '현재 UI, 버전 표기, 공지 패널, 로컬 저장 흐름도 다시 점검해 스타일과 다국어 구조를 그대로 유지했습니다.',
  ...
  fr: {
    ...
    announcementDescription: 'La version 0.3.3.1 ajoute un nouveau passage de stabilité : les échecs de téléchargement paper2gal utilisent désormais eux aussi un diagnostic lisible.',
    announcementList1: 'Si le téléchargement d’un fichier ou de l’archive complète échoue, l’interface affiche maintenant la cause lisible, la réponse backend et l’URL réelle de la requête.',
    announcementList2: 'La détection d’un endpoint modèle mal renseigné couvre maintenant le lancement, le polling et le téléchargement.',
    announcementList3: 'L’UI actuelle, le numéro de version, les panneaux d’annonce et la persistance locale ont été revérifiés pour garder le style et les langues synchronisés.',
    pageFaceTitle: 'Face Maker',
  ...
  de: {
    ...
    announcementDescription: 'Version 0.3.3.1 ergänzt eine weitere Stabilitätsrunde: Auch paper2gal-Downloadfehler werden jetzt mit lesbarer Diagnose dargestellt.',
    announcementList1: 'Wenn der Download einzelner Dateien oder des gesamten Archivs fehlschlägt, zeigt die UI nun Ursache, Backend-Antwort und die tatsächliche Request-URL an.',
    announcementList2: 'Die Erkennung falsch eingetragener Modell-Endpunkte deckt jetzt Start, Polling und Download ab.',
    announcementList3: 'Die aktuelle UI, Versionsanzeige, Ankündigungspanels und lokale Persistenz wurden noch einmal geprüft, damit Stil und Sprachen konsistent bleiben.',
    pageFaceTitle: 'Face Maker',
  ...
  es: {
    ...
    announcementDescription: 'La versión 0.3.3.1 añade otra ronda de estabilidad: los fallos de descarga en paper2gal ahora también muestran un diagnóstico legible.',
    announcementList1: 'Si falla la descarga de un archivo individual o del paquete completo, la interfaz ahora enseña la causa, la respuesta del backend y la URL real de la petición.',
    announcementList2: 'La detección de endpoints de modelo mal pegados ahora cubre inicio, sondeo y descarga.',
    announcementList3: 'Se volvió a revisar la UI actual, el número de versión, los paneles de anuncio y la persistencia local para mantener sincronizados estilo e idiomas.',
    pageFaceTitle: 'Face Maker',
  ...
  it: {
    ...
    announcementDescription: 'La versione 0.3.3.1 aggiunge un ulteriore passaggio di stabilità: anche gli errori di download paper2gal ora vengono mostrati con una diagnosi leggibile.',
    announcementList1: 'Se fallisce il download di un singolo file o dell’archivio completo, l’interfaccia mostra ora causa, risposta del backend e URL reale della richiesta.',
    announcementList2: 'Il rilevamento degli endpoint modello incollati per errore copre ora avvio, polling e download.',
    announcementList3: 'UI attuale, numero versione, pannelli avvisi e persistenza locale sono stati ricontrollati per mantenere coerenti stile e lingue.',
    pageFaceTitle: 'Face Maker',
    pageStyleTitle: 'Style transfer',
    pagePromptTitle: 'Character Prompt + LLM / TTS',
  ...
  pt: {
    ...
    announcementDescription: 'A versão 0.3.3.1 acrescenta outra rodada de estabilidade: falhas de download no paper2gal agora também aparecem com diagnóstico legível.',
    announcementList1: 'Se o download de um arquivo individual ou do pacote completo falhar, a interface agora mostra a causa, a resposta do backend e a URL real da requisição.',
    announcementList2: 'A detecção de endpoint de modelo colado por engano agora cobre início, polling e download.',
    announcementList3: 'A UI atual, o número da versão, os painéis de anúncio e a persistência local foram revisados novamente para manter estilo e idiomas sincronizados.',
    pageFaceTitle: 'Face Maker',
