...
const translations: Record<BaseLanguage, Messages> = {
  ...
  workflowFormats: '支持 PNG / JPG / WEBP，推荐上传单人立绘或清晰半身图。',
    ...
    startModalDescription: '先进入你现在要使用的主入口之一。',
    ...
    announcementDescription: 'v1.18.2 第二轮全面自查修复：场景剧本工坊本地化完整性、SFX double-play 根除、可访问性与代码质量进一步提升。',
    announcementList1: '本地化完整性：修复 saveToast、副本后缀、未命名场景标题、Markdown 页脚等 4 处硬编码字符串，全部纳入 5 语言翻译系统。',
    announcementList2: 'SFX 根除 double-play：为场景列表项（role=button）补全 data-sfx-handled，彻底消除全局 handler 与显式 onClick 的叠加播放。',
    announcementList3: '可访问性与代码质量：集成 confirmDestructiveActions 设置到 deleteScene/deleteLine；Escape 键关闭导出模态框；触摸设备操作按钮始终可见；移除死代码 fileInputRef/handleImport。',
    ...
    docsFilterAll: '全部',
    docsFilterCritical: '严重',
    docsFilterError: '错误',
    docsFilterWarning: '警告',
    docsFilterInfo: '提示',
    docsErrorCount: '共 {count} 条错误',
    docsErrorCause: '原因',
    docsErrorSolution: '解决方法',
    docsErrorSteps: '解决步骤',
    docsErrorRelated: '相关错误',
    docsErrorPrevention: '预防措施',
  ...
  ja: {
    ...
    workflowHint: '開始ボタンを押すと主入口モーダルが開き、上のカードから直接入ることもできます。',
    ...
    startModalDescription: '今使いたい主入口から 1 つ選んでください。',
    ...
    announcementDescription: 'v1.18.2 第二回全面的な自己チェック修正：シーン脚本スタジオのローカライズ完全性、SFX double-play 根絶、アクセシビリティとコード品質のさらなる向上。',
    announcementList1: 'ローカライズ完全性：saveToast、コピー接尾辞、無名シーンタイトル、Markdown フッターなど 4 箇所のハードコード文字列を修正し、全 5 言語翻訳システムに統合。',
    announcementList2: 'SFX double-play 根絶：シーンリスト項目（role=button）に data-sfx-handled を補完し、グローバル handler と明示的 onClick の重複再生を完全に排除。',
    announcementList3: 'アクセシビリティとコード品質：deleteScene/deleteLine に confirmDestructiveActions 設定を統合；Escape キーでエクスポートモーダルを閉じる；タッチデバイスで操作ボタンを常時表示；fileInputRef/handleImport デッドコードを削除。',
    ...
    docsFilterAll: 'すべて',
    docsFilterCritical: '深刻',
    docsFilterError: 'エラー',
    docsFilterWarning: '警告',
    docsFilterInfo: '情報',
    docsErrorCount: '合計 {count} 件',
    docsErrorCause: '原因',
    docsErrorSolution: '解決方法',
    docsErrorSteps: '解決手順',
    docsErrorRelated: '関連エラー',
    docsErrorPrevention: '予防策',
  ...
  en: {
    ...
    workflowHint: 'Press Start to open the launcher, or click any icon card above to jump in directly.',
    ...
    startModalDescription: 'Select a workbench to enter first.',
    ...
    announcementDescription: 'v1.18.2 Second-round comprehensive self-audit fixes: Scene Writer localization completeness, SFX double-play elimination, accessibility and code quality further improved.',
    announcementList1: 'Localization completeness: fixed 4 hardcoded strings — saveToast, copy suffix, untitled scene title, Markdown footer — all now integrated into the 5-language translation system.',
    announcementList2: 'SFX double-play eliminated: added data-sfx-handled to scene list items (role=button), completely removing overlapping playback between global handler and explicit onClick.',
    announcementList3: 'Accessibility & code quality: integrated confirmDestructiveActions setting into deleteScene/deleteLine; Escape key closes export modal; action buttons always visible on touch devices; removed dead code fileInputRef/handleImport.',
    ...
    docsFilterAll: 'All',
    docsFilterCritical: 'Critical',
    docsFilterError: 'Error',
    docsFilterWarning: 'Warning',
    docsFilterInfo: 'Info',
    docsErrorCount: '{count} errors total',
    docsErrorCause: 'Cause',
    docsErrorSolution: 'Solution',
    docsErrorSteps: 'Steps',
    docsErrorRelated: 'Related',
    docsErrorPrevention: 'Prevention',
  ...
  ru: {
    ...
    workflowHint: 'Кнопка старта открывает модальное окно с основными входами, но можно зайти и напрямую по карточке.',
    ...
    startModalDescription: 'Сначала выберите один из основных рабочих столов.',
    ...
    announcementDescription: 'v1.18.2 Исправления при втором полном самоаудите: полнота локализации студии сценариев, искоренение double-play SFX, дальнейшее улучшение доступности и качества кода.',
    announcementList1: 'Полнота локализации: исправлены 4 жёстко закодированные строки — saveToast, суффикс копии, заголовок безымянной сцены, нижний колонтитул Markdown — всё интегрировано в 5-язычную систему перевода.',
    announcementList2: 'Искоренение double-play SFX: добавлен data-sfx-handled на элементы списка сцен (role=button), полностью устранено наложение воспроизведения между глобальным handler и явным onClick.',
    announcementList3: 'Доступность и качество кода: интегрирована настройка confirmDestructiveActions в deleteScene/deleteLine; закрытие модального окна экспорта по Escape; кнопки действий всегда видны на сенсорных устройствах; удалён мёртвый код fileInputRef/handleImport.',
    ...
    docsFilterAll: 'Все',
    docsFilterCritical: 'Критические',
    docsFilterError: 'Ошибки',
    docsFilterWarning: 'Предупреждения',
    docsFilterInfo: 'Информация',
    docsErrorCount: 'Всего ошибок: {count}',
    docsErrorCause: 'Причина',
    docsErrorSolution: 'Решение',
    docsErrorSteps: 'Шаги',
    docsErrorRelated: 'Связанные',
    docsErrorPrevention: 'Профилактика',
...
const localizedMessages: Record<AppLanguage, Messages> = {
  ...
  ko: {
    ...
    announcementDescription: 'v1.18.2 두 번째 전면 자체 점검 수정: 장면 각본 공방 현지화 완성도、SFX double-play 근절、접근성 및 코드 품질 추가 향상。',
    announcementList1: '현지화 완성도: saveToast、복사본 접미사、제목 없는 장면 제목、Markdown 바닥글 등 4곳의 하드코딩 문자열을 수정하여 전체 5개 언어 번역 시스템에 통합。',
    announcementList2: 'SFX double-play 근절: 장면 목록 항목(role=button)에 data-sfx-handled를 보완하여 전역 handler와 명시적 onClick의 중복 재생을 완전히 제거。',
    announcementList3: '접근성 및 코드 품질: deleteScene/deleteLine에 confirmDestructiveActions 설정 통합；Escape 키로 내보내기 모달 닫기；터치 디바이스에서 조작 버튼 항상 표시；fileInputRef/handleImport 데드코드 제거。',
...
const announcementHistory = [
  ...
  },
  {
    version: '1.18.1',
    date: '2026-05-18',
    title: '1.18.1 场景剧本工坊 bug 修复与体验优化',
    summary:
      'SFX 合规、可访问性、移动端适配、存储失败提示与键盘交互全面补齐。',
    details: [
      'SFX 合规：为全部显式调用 playSound 的按钮补全 data-sfx-handled，彻底消除 copy/delete/line-delete 等按钮的 double-play。',
      '可访问性与交互：场景列表项新增 role/tabIndex/onKeyDown/aria-label 支持键盘导航；导出模态框补全 role="dialog"/aria-modal/Escape 关闭；集成 confirmDestructiveActions 设置；自动保存失败时显示 toast 提示。',
      '移动端与本地化：触摸设备上场景操作按钮始终可见；随机生成场景的角色名与情绪标签改为跟随界面语言；拖拽排序补全 dataTransfer 兼容性；移除未使用的 fileInputRef/handleImport 死代码。',
    ],
  },
  {
    version: '1.17.0',
  ...
  },
  {
    version: '1.12.7',
    date: '2026-05-18',
    title: '1.12.7 第五轮全局深度审计：SFX/BGM/本地化/可访问性',
    summary:
      '对 v1.12.6 进行最严格的第五轮全局深度审计，修复 7 处 modal-close double-play、BGM 混响内存泄漏、SFX ADSR/Pan 死设置、ImportModal JSON 防御、多页面本地化与可访问性缺失。',
    details: [
      'SFX 修复：移除 7 个页面 modal-close 按钮上的重复 inline playSound，避免与全局 handler 重叠播放；CharacterStatsDesignerPage/SkillTreePage 按钮确认为 false positive（函数内已有音效）。',
      'BGM 引擎修复：audioEngine.ts rebuildMusicChain() 在清理旧混响链时释放 convolver.buffer，消除反复拖动混响滑块导致的内存泄漏；synthesize() 接入 sfxAttack/Decay/Sustain/Release/Pan 用户设置，修复高级音频参数完全失效的问题。',
      'ImportModal 加固：validateAndImport 增加 JSON.parse 结果类型校验，防御原始值/数组输入导致的 TypeError。',
      '本地化修复：CharacterBattleCardPage/SkillTreePage 统一使用 backHome prop 替代本地 labels；DocsPage 补全 clearSearch 翻译键与 5 语翻译；CharacterBattleCardPage "Exporting…" 硬编码补全 5 语翻译；AudioEditorPage/AudioConverterPage 核心按钮与状态文本（Play/Pause/Stop/Export/Convert/Download 等）补全 5 语本地 labels。',
      '可访问性修复：CharacterBattleCardPage 名称/称号输入框、WorldEncyclopediaPage 搜索框、DocsPage 搜索框补全 aria-label。',
    ],
  },
  {
    version: '1.12.6',
  ...
  },
  {
    version: '1.12.4',
    date: '2026-05-18',
    title: '1.12.4 第三轮全局深度审计',
    summary:
      '对全局进行第三轮深度审计，修复 WorldEncyclopedia 导入计数错误、两处硬编码英文未本地化、Modal 标题不区分添加/编辑、筛选按钮缺少 aria-pressed、pageTitle 未渲染、标签可重复添加、AudioEditor/AudioConverter 返回按钮无声、ImportModal 缺失 character-gif 和 index-tts 导入支持等。',
    details: [
      'WorldEncyclopediaPage：修复导入成功 toast 错误地计算重复条目（现在显示实际新增数量）；本地化 "No matching entries" 和 "No characters in Relationship Web"；Modal 标题根据 add/edit 模式动态切换；筛选 chip 和视图切换按钮添加 aria-pressed；渲染 pageTitle/pageDescription；导入时验证 tool 字段防止误导入其他工具数据；addTag 添加重复标签防护。',
      'AudioEditorPage & AudioConverterPage：修复返回按钮缺少 playSound("back") 和 data-sfx-handled；AudioConverter 的 "Audio Editor" 切换按钮添加 playSound("pageSwitch")。',
      'App.tsx ImportModal：修复 "Go to Tool" 按钮双重音效；新增 index-tts 到可导入工具列表；为 character-gif 和 index-tts 补全 importToolConfig case；导入失败时显示具体错误原因（而非通用 "Tool mismatch"）。',
    ],
  },
  {
    version: '1.12.3',
    date: '2026-05-18',
    title: '1.12.3 全局深度审计与稳定性修复',
    summary:
      '对全局进行第二轮深度审计，修复 WorldEncyclopedia 导入崩溃、技能树/战斗卡按钮完全无声、ImportModal 双重音效、导出格式标准化等关键问题。',
    details: [
      'WorldEncyclopediaPage：新增 JSON 导入功能（支持标准对象格式与直接数组）；修复 handleImport 中未定义函数 persist 导致的崩溃；导出 JSON 添加 tool 字段以兼容 ImportModal；新增 normalizeEntry 数据归一化，防御性处理缺失的 tags/relatedCharacterIds；验证 category 有效性。',
      'CharacterSkillTreePage & CharacterBattleCardPage：修复无效音效名称 ui-click（audioEngine 中不存在该预设）导致全部相关按钮完全无声的 bug，统一替换为 buttonClick。',
      'App.tsx ImportModal：新增 world-encyclopedia 导入支持；修复 action-tile 和 validate 按钮的双重音效播放（内联 playSound 与全局 handler 冲突）；同步更新 screenMap 与 tools 列表。',
    ],
  },
  {
    version: '1.12.2',
  ...
  },
  {
    version: '1.8.0',
    date: '2026-05-16',
    title: '1.8.0 新增角色编年史（Character Chronicle）',
    summary:
      '全新上线 OC 角色编年史功能，支持为角色创建可视化时间线，记录诞生、相遇、成长、战斗、转折等关键事件。7 种事件类型颜色编码、关系网角色联动、资产库图片附件，一键导出长图 PNG。',
    details: [
      '角色编年史：为任意角色创建按日期排序的可视化时间线，每个事件包含日期、标题、描述、类型、关联角色和可选图片。事件类型包括诞生（橙）、相遇（绿）、离别（红）、成长（蓝）、战斗（橙红）、转折点（紫）、自定义（灰），每种类型都有独特的颜色编码。',
      '深度数据联动：自动读取角色关系网中的角色列表，可将任意角色关联到特定事件；事件卡片上会显示关联角色的彩色圆点。支持从资产库选择图片作为事件配图，在时间线卡片中直接展示。',
      '可视化时间轴：采用经典垂直时间轴设计，事件卡片沿中线左右交替排列，日期自动排序。桌面端和移动端均做了精心适配——移动端改为单侧左对齐布局，确保小屏幕阅读体验。',
      '一键导出：基于 html-to-image 以 2 倍分辨率将完整时间轴渲染为透明背景 PNG 长图，可直接用于社交媒体长文、创作展示或印刷。所有数据自动保存到 localStorage。',
      '完整集成：新增 FeatureScreen 路由、ActionIcon SVG、StartModal 入口、HomeScreen 工作流列表；5 语言完整翻译和手册文档；全页面 SFX 音效覆盖。',
    ],
  },
  {
  ...
  },
  {
    version: '1.7.8',
    date: '2026-05-16',
    title: '1.7.8 角色关系网深度审计与全面修复',
    summary:
      '对 v1.7.7 新增的角色关系网进行全面代码审计，修复 20+ 项关键缺陷：触摸事件支持、性能优化、可访问性增强、移动端适配、文档补全、翻译补全、数据验证强化。',
    details: [
      '性能优化：edgePaths 计算从 O(n×m) 优化为 O(n+m)（使用 Map 预计算节点索引）；galleryCache 查找从 O(n) 线性搜索优化为 O(1) Map 查找；useBeforeUnloadGuard 现在同时监听节点和边的变化。',
      '移动端支持：为画布和节点添加完整的触摸事件处理器（touchStart/touchMove/touchEnd），支持单指平移画布和拖拽节点；将移动端侧边栏从「完全隐藏」改为底部抽屉（bottom sheet），选中节点/边时自动滑出。',
      '可访问性增强：所有交互元素（节点、画布按钮、边列表项、颜色色板、关系类型 chip、头像选择项）新增 focus-visible 焦点环；节点支持键盘 Enter/Space 选中、Shift+Enter 快速添加关系；模态框输入框自动聚焦；颜色色板和关系 chip 添加 aria-pressed 状态。',
      '数据验证强化：loadEdges 增加 type/label/notes 字段验证，并对无效 type 自动回退到 custom；loadNodes 已包含 color 校验。',
      '文档与翻译补全：为 relationship-web 在 5 种语言（中/日/英/俄/韩）的 docsContent 中补充完整的手册条目；韩语翻译补全 actionRelationshipWeb；移除所有语言中过时的「七个」「11 个工具」硬编码计数文案。',
      'CSS 修复：修复 .rel-canvas flex:1 但父容器未设置 display:flex 的关键布局问题；将 box-shadow 脉冲动画替换为 transform 缩放动画以消除重绘开销；头像边框从硬编码 rgba(255,255,255,0.12) 改为 theme-aware 的 var(--border)。',
    ],
  },
  {
    version: '1.7.7',
  ...
  },
  {
    version: '1.7.4',
    date: '2026-05-12',
    title: '1.7.4 音频页面深度 bug 修复与稳定性提升',
    summary:
      '全面修复 AudioEditorPage 和 AudioConverterPage 的运行时崩溃、内存泄漏、stale closure、拖拽闪烁等 20+ 项严重问题；修复 App.tsx hover SFX 失效和 FaceMakerPage 键盘快捷键冲突；新增音频工具专用 CSS 类与无障碍支持。',
    details: [
      'AudioEditorPage 运行时修复：修复 dragCounter 未声明导致的拖拽 ReferenceError；修复 stopPlayback 在 handleImport 之后定义导致的 Temporal Dead Zone 崩溃；修复 tick/onended 中的 stale closure（speed/loop/buffer 在播放中修改后不生效）；修复每次播放创建的 ~10 个 audio node 未 disconnect 导致的内存泄漏；修复键盘快捷键 effect 依赖 currentTime 每帧重建（60fps add/removeEventListener）；修复导出 URL 在组件卸载时未 revoke；修复 resetAll 未 revoke 导出 URL。',
      'AudioEditorPage 交互修复：修复 applyFade 淡入/淡出 off-by-one（最后样本 gain 永远不到目标值）；修复 history 数组无上限导致内存爆炸（新增 MAX_HISTORY=50）；修复拖拽时 dragleave 在子元素间触发导致的闪烁；修复拖拽文件 MIME 类型为 application/octet-stream 时被错误拒绝；修复 Canvas fillRect 可能获得负宽度；修复 handleWheel 除以零；修复空格键长按导致 playback 高频切换。',
      'AudioConverterPage 稳定性修复：修复异步操作（import/convert）在组件卸载后继续 setState 的 React 警告；修复 error panel 下载使用 requestAnimationFrame 在后台标签页导致 blob URL 泄漏；修复 e.dataTransfer.types.includes("Files") 在旧浏览器中 crash；修复 Reset 按钮 double-play resetSound；修复 Convert 按钮在导入期间未禁用；修复 sliderThrottleRef 未在 handleReset 中清除。',
      'App 层修复：修复全局 hover SFX 使用 mouseenter（不冒泡）导致音效完全失效，改为 mouseover；修复 FaceMakerPage 全局 keyboard shortcut 未检查 select 元素和 shiftKey；添加 stopPropagation 防止快捷键事件冒泡。',
      'CSS 与无障碍修复：修复 --accent 未定义导致拖拽高亮边框和 splash 百分比文字失效；修复 --text-muted 在 4 处被引用但从未定义；新增 .upload-dropzone、.audio-waveform-surface 等音频工具专用样式；优化 .empty-state 对比度；上传区添加 tabIndex/role/aria-label 支持键盘导入。',
    ],
  },
  {
    version: '1.7.3',
  ...
  },
  {
    ...
    details: [
      ...
      'Paper2Gal 操作优化：执行中的表情、CG 和抠图步骤也可以发起单结果重做；后端使用 run_id 防止旧任务完成后覆盖新重做结果。',
      'Zeabur 本地上传流程补齐：新增 package:zeabur 脚本和最小上传目录生成脚本，避免上传整个项目时带入 node_modules、dist、tmp、.env 和旧 zip。',
  ...
  },
  {
    ...
    details: [
      ...
      '新增「全错误索引」页面：侧边栏一键进入，顶部筛选栏支持按严重等级（全部/严重/错误/警告/提示）过滤，每条错误显示为紧凑索引行，实时显示数量徽章。',
      '新增「预防措施」字段：为全部 79 个 critical/error 级别的错误条目添加 prevention 字段，在用户遇到错误之前提供预防建议（如「避免在无痕模式中使用」「定期检查 API Key 有效期」），实现从「事后修复」到「事前预防」的文档升级。',
      '新增 7 个本地化键：docsNavIndex（全错误索引）、docsFilterAll/Critical/Error/Warning/Info（筛选按钮）、docsErrorCount（错误计数），中/日/英/俄四语同步翻译。',
      '新增 8 个本地化键：docsNavDictionary（错误字典）、docsErrorSeverity（严重程度）、docsErrorCategory（分类）、docsErrorLocation（位置）、docsErrorSteps（解决步骤）、docsErrorRelated（相关错误）、docsErrorPrevention（预防措施），中/日/英/俄四语同步翻译。',
      '新增 CSS 样式：severity-critical/error/warning/info 四级颜色标签、docs-error-meta 元信息面板、docs-error-steps 编号列表、docs-error-related 相关错误标签云、docs-index-filters 筛选栏、docs-error-prevention 预防提示框。',
  ...
  useEffect(() => {
    function onKeyDown(e: KeyboardEvent) {
      ...
      if (e.ctrlKey || e.metaKey) {
        ...
        else if (e.key === 's') { e.preventDefault(); e.stopPropagation(); saveDraftRef.current(); }
        else if (e.key === 'e') { e.preventDefault(); e.stopPropagation(); void exportPngRef.current(); }
        else if (e.key === 'r') { e.preventDefault(); e.stopPropagation(); randomizeDraftRef.current(); }
      }
      // Preset hotkeys (1-4)
      if (!e.ctrlKey && !e.metaKey && !e.altKey && !e.shiftKey) {
        if (e.key === '1') { e.preventDefault(); e.stopPropagation(); applyPresetRef.current('cute'); }
        else if (e.key === '2') { e.preventDefault(); e.stopPropagation(); applyPresetRef.current('mature'); }
        else if (e.key === '3') { e.preventDefault(); e.stopPropagation(); applyPresetRef.current('cool'); }
        else if (e.key === '4') { e.preventDefault(); e.stopPropagation(); applyPresetRef.current('sporty'); }
      }
    }
        ...
        {step === 'select' ? (
          ...
          <>
            ...
            <div className="workflow-actions" style={{ marginTop: 16 }}>
              ...
              {status === 'success' && (
                <button className="secondary-button" type="button" data-sfx-handled onClick={goToTool}>
