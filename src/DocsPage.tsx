...
const severityLabelKeyMap: Record<string, keyof DocsLabels> = {
  critical: 'docsFilterCritical',
  error: 'docsFilterError',
  warning: 'docsFilterWarning',
  info: 'docsFilterInfo',
};
...
function ErrorCard({ error, messages, isHighlighted }: { error: import('./docsContent').DocsErrorItem; messages: DocsLabels; isHighlighted?: boolean }) {
  ...
  const severityLabel = messages[severityLabelKeyMap[error.severity]] ?? error.severity;
      ...
      <div className="docs-index-list">
        {filtered.map((err, i) => (
          <div key={i} className={`docs-index-row ${severityClassMap[err.severity] ?? ''}`}>
            <span className="docs-index-severity">{messages[severityLabelKeyMap[err.severity]] ?? err.severity}</span>
